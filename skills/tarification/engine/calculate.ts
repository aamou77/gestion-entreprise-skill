import type { FormulaId, PricingInput, PricingResult, ResultKey } from "./types.ts";
import { validatePricingInput } from "./validate.ts";

function unavailableResult(input: PricingInput, errors: PricingResult["errors"]): PricingResult {
  return {
    status: "unavailable",
...
function retainedInputs(input: PricingInput): Partial<PricingInput> {
  const common = {
    mode_analyse: input.mode_analyse,
    mode_calcul: input.mode_calcul,
    periode: {
...
function addResult(
  result: PricingResult, key: ResultKey, value: number, formulaId: FormulaId, inputsUsed: string[],
): void {
...
export function calculatePricing(input: PricingInput): PricingResult {
  ...
  if (errors.length > 0) return unavailableResult(input, errors);
  ...
  switch (input.mode_calcul) {
    case "analyse_prix": {
      ...
      if (result.indisponibles.length > 0) result.status = "partial";
      break;
    }
    case "prix_minimum_economique":
      ...
      break;
    case "prix_cible_taux_marque":
      addResult(result, "prix_cible", input.cout! / (1 - input.taux_marque_cible!),
        "TARGET_PRICE_FROM_MARK_RATE", ["cout", "taux_marque_cible"]);
      break;
    case "prix_cible_taux_marge":
      addResult(result, "prix_cible", input.cout! * (1 + input.taux_marge_cible!),
        "TARGET_PRICE_FROM_MARGIN_RATE", ["cout", "taux_marge_cible"]);
      break;
    case "remise":
      addResult(result, "montant_remise", input.prix_initial! * input.taux_remise!,
        "DISCOUNT_AMOUNT", ["prix_initial", "taux_remise"]);
      addResult(result, "prix_apres_remise", input.prix_initial! * (1 - input.taux_remise!),
        "PRICE_AFTER_DISCOUNT", ["prix_initial", "taux_remise"]);
      break;
  }
