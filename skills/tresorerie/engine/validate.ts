import type { AnalysisMode, TreasuryInput, ValidationError } from "./types.ts";

const statuses = new Set(["realise", "engage", "attendu", "hypothetique"]);

function add(errors: ValidationError[], code: ValidationError["code"], fields: string[], message: string): void {
...
export function validateTreasuryInput(input: TreasuryInput): { errors: ValidationError[] } {
  ...
  const mode = candidate.mode_analyse as AnalysisMode;
  if (mode !== "reel" && mode !== "scenario") {
    add(errors, "MISSING_REQUIRED_INPUT", ["mode_analyse"], "mode_analyse doit être reel ou scenario.");
  }
  if (!nonEmptyString(candidate.devise)) add(errors, "INVALID_CURRENCY", ["devise"], "devise doit être une chaîne non vide.");
  ...
  candidate.periodes.forEach((periodValue, periodIndex) => {
    ...
    if (!nonEmptyString(period.id)) add(errors, "INVALID_PERIOD", [`${path}.id`], "id de période doit être une chaîne non vide.");
    ...
    if (range === null || typeof range !== "object" || Array.isArray(range)) {
      ...
      const dates = range as Record<string, unknown>;
    ...
    period.flux.forEach((flowValue, flowIndex) => {
      ...
      if (!nonEmptyString(flow.id)) add(errors, "INVALID_FLOW_ID", [`${flowPath}.id`], "id doit être une chaîne non vide.");
      else if (flowIds.has(flow.id)) add(errors, "DUPLICATE_FLOW_ID", [`${flowPath}.id`], "id de flux dupliqué.");
      else flowIds.add(flow.id);
      if (flow.sens !== "encaissement" && flow.sens !== "decaissement") add(errors, "INVALID_FLOW_DIRECTION", [`${flowPath}.sens`], "sens de flux invalide.");
      if (!statuses.has(flow.statut as string) || (mode === "reel" && flow.statut === "hypothetique")) {
        add(errors, "INVALID_FLOW_STATUS", [`${flowPath}.statut`], "statut de flux invalide pour le mode.");
      }
      if (!isFiniteNumber(flow.montant) || flow.montant < 0) add(errors, "INVALID_FLOW_AMOUNT", [`${flowPath}.montant`], "montant doit être un nombre fini positif ou nul.");
      if (!isIsoDate(flow.date)) add(errors, "INVALID_FLOW_DATE", [`${flowPath}.date`], "date doit être ISO YYYY-MM-DD.");
