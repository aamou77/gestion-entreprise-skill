import type { FormulaId, ResultKey, TreasuryFlow, TreasuryInput, TreasuryPeriodResult, TreasuryResult } from "./types.ts";
import { validateTreasuryInput } from "./validate.ts";
...
function copyFlow(flow: TreasuryFlow): TreasuryFlow { return { ...flow }; }
function canonicalFlows(flows: TreasuryFlow[]): TreasuryFlow[] {
  return [...flows].sort((left, right) => (left.id < right.id ? -1 : left.id > right.id ? 1 : 0));
}
function canonicalFlowIds(flows: TreasuryFlow[]): string[] {
  return canonicalFlows(flows).map(flow => flow.id);
}
function unavailable(input: TreasuryInput, errors: TreasuryResult["errors"]): TreasuryResult {
  return { status: "unavailable", mode_analyse: input?.mode_analyse, inputs_retenus: {}, periodes: [], warnings: [], errors };
}
function addResult(
  period: TreasuryPeriodResult, key: ResultKey, value: number, formula: FormulaId, inputs: string[], flowIds?: string[],
): void {
  period.resultats[key] = value;
  period.provenance[key] = { formula_id: formula, inputs_used: inputs, ...(flowIds !== undefined ? { flow_ids: flowIds } : {}) };
}
function copyInput(input: TreasuryInput): TreasuryInput {
  return {
...
function finiteResults(result: TreasuryResult): boolean {
...
export function calculateTreasury(input: TreasuryInput): TreasuryResult {
  ...
  if (errors.length > 0) return unavailable(input, errors);
  const result: TreasuryResult = {
  ...
  for (const periodInput of input.periodes) {
    const period: TreasuryPeriodResult = {
    ...
    for (const flow of periodInput.flux) {
      if (flow.date < periodInput.periode.debut || flow.date > periodInput.periode.fin) period.flux_exclus.push({ id: flow.id, code: "OUTSIDE_PERIOD" });
      else period.flux_retenus.push(copyFlow(flow));
    }
    for (const [status, inflowKey, outflowKey, inflowFormula, outflowFormula] of statusConfig) {
      if (status === "hypothetique" && input.mode_analyse === "reel") continue;
    ...
    if (input.reserve_tresorerie !== undefined) {
      if (input.mode_analyse === "reel") addResult(period, "solde_disponible_apres_attendus_et_reserve", r.solde_apres_attendus! - input.reserve_tresorerie, "AVAILABLE_BALANCE_AFTER_EXPECTED_AND_RESERVE", ["solde_apres_attendus", "reserve_tresorerie"]);
      else addResult(period, "solde_disponible_scenario_apres_reserve", r.solde_scenario! - input.reserve_tresorerie, "AVAILABLE_SCENARIO_BALANCE_AFTER_RESERVE", ["solde_scenario", "reserve_tresorerie"]);
    }
    result.periodes.push(period);
  }
  if (!finiteResults(result)) return unavailable(input, [{ code: "INVALID_NUMBER", fields: ["resultats"], message: "Le calcul dépasse la capacité numérique de number." }]);
  return result;
}
